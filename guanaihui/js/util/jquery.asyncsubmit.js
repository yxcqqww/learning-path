/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
1. 插件名称：asyncsubmit
2. 插件描述：表单以异步提交
3. 版本：1.0
4.  对其他插件的依赖：验证控件来处理表单验证
5. 备注：    
6. 未尽事宜：
7. 作者：赵华刚(zhaohuagang@guanaihui.com)
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
(function($){
    $.fn.asyncsubmit = function(options){		
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        将每个asyncsubmit加上对应的配置属性
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        var opts = $.extend({}, $.fn.asyncsubmit.defaults, options) ;
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        执行事件添加后返回
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        return this.each(function() {    
            /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            根据表单的class里面是否含有validate来判断是否需要验证
            -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
            var validate = $(this).hasClass("validate") ;
            var form = this ;
            if(validate) {
                 require([
                    opts.utilStaticPrefix + "/validation/js/languages/jquery.validationEngine-zh_CN.js" ,
                    opts.utilStaticPrefix + "/validation/js/jquery.validationEngine.js"
                ], function(){
                    $(form).validationEngine({
                        promptPosition : opts.promptPosition ,
                        autoHidePrompt : true ,
                        autoHideDelay : 5000 ,
                        scroll : opts.scroll
                    }) ;
                }) ;
            }
            /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            根据表单的class里面是否含有validate来判断是否需要验证
            -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
            // $(this).find(opts.submitBtnSelector).click(function(){
            //     $(form).trigger("submit") ;
            // });
            
            $(this).submit(function(){                
                /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                先执行保存前的接口方法
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                if(opts.beforeSaveInterface) opts.beforeSaveInterface() ;
                /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                再验证，如果通过再保存
                -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
                if($(this).validationEngine('validate')) {                  
                    $.fn.asyncsubmit.save(this, opts) ;                    
                } 
                return false;
            });
        });
    };
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    保存处理的方法
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    $.fn.asyncsubmit.save = function(form, opts) {   
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        参数获取：
        1. 提交处理的url如果实例化的时候没有给出，就从form标签的action属性中获取
        2. 请求是post还是get也是如果实例化的时候没有给出，从form的method属性中获取
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        var url = opts.apiUrl ? opts.apiUrl : $(form).attr("action") ;        
        var type = opts.type ? opts.type : (($(form).attr("method") === undefined || $(form).attr("method") === "") ? "GET" : $(form).attr("method").toUpperCase()) ;
        if(opts.dataType === "jsonp") type = "GET" ;
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        根据配置决定是否需要对表单所有价格类控件进行四舍五入处理
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        if(opts.priceRound) {
            $(form).find("." + opts.priceClassName).each(function(){
                var value = parseFloat($(this).val()).toFixed(opts.pricePrecision) ;
                $(this).val(value) ;
            }) ;
        }
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        执行过程中的接口方法
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/        
        if(opts.onSavingInterface) opts.onSavingInterface() ;
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        执行过程中的接口方法
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/  
        var conf = {
            url : url ,
            data : $(form).serialize() ,
            type : type ,
            dataType : opts.dataType ,            
            beforeSend: function(request) {
                request.setRequestHeader("X-Requested-With", "XMLHttpRequest");
            } ,
            error : function(e) {
                opts.onErrorInterface() ;
            } ,
            success : function(data){
                if(data.code.toString() === "200") opts.onSuccessInterface(data.result) ;
                else opts.onExceptionInterface(data.message) ;             
            }
        } ;
        if(opts.dataType === "jsonp") conf.jsonpCallback = "callback" ;  //这个配置是在没有真正后端接口前端用自己的 json文件模拟接口的时候为了保持callback参数值一致所做的设置
         /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        发起ajax请求
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        try {
            $.ajax(conf); 
        }
        catch(e) {
            console.log("错误名称："+e.name+"\n错误描述："+e.message);
        }
    
    };
    /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    asyncsubmit默认配置
    -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    $.fn.asyncsubmit.defaults = {
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        接口地址
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        apiUrl : "" ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        静态资源工具路径
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        utilStaticPrefix : "dev01.guanaitui.cn/guanaihui/js/util" ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        提交按钮选择器
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        submitBtnSelector : "[type='submit']" ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        保存前的接口方法
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        beforeSaveInterface : null ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        保存过程中的接口方法
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        onSavingInterface : null ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        请求接口出错的时候的接口方法
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        onErrorInterface : null ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        成功获取数据后的接口方法，这个方法中的唯一参数就是以结果json中的result这个key的内容
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        onSuccessInterface : null ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        成功获取数据但是出现异常后的接口方法，这个方法中的唯一参数就是以结果json中的message这个key的内容
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        onExceptionInterface : null ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        表单提交的方式，默认为POST
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        type : "POST" ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        保存接口返回的数据类型，默认为json
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        dataType : "json" ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        是否需要处理价格信息成保留两位小数
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        priceRound : true ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        价格表单控件的类名
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        priceClassName : "price-round" ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        价格精度，就是价格小数位数，默认为2
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        pricePrecision : 2 ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        验证控件错误信息提示位置
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        promptPosition : "topLeft" ,
        /*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
        验证控件错误信息提示的时候页面是否滚动
        -----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
        scroll : true
    };
})(jQuery) ;